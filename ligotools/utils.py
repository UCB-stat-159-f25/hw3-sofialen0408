...
def write_wavfile(folder,filename,fs,data):
    d = np.int16(data/np.max(np.abs(data)) * 32767 * 0.9)
    wavfile.write(f"{folder}/{filename}",int(fs), d)
...
def plot_detector_results(pcolor, strain_whitenbp, time, timemax, tevent, template_match, 
			  template_fft, datafreq, freqs, data_psd, fs, SNR, det, d_eff, 
			  eventname, fig_dir, plottype):
	...
	plt.xlabel('Time since {0:.4f}'.format(timemax))
	plt.legend(loc='upper left')
	plt.savefig(f"{fig_dir}/{eventname}_{det}_SNR.{plottype}")
	...
	plt.savefig(f"{fig_dir}/{eventname}_{det}_matchtime.{plottype}")
	...
	plt.savefig(f"{fig_dir}/{eventname}_{det}_matchfreq.{plottype}")
